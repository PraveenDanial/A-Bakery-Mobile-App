package com.example.myapplication.DBClasses;
...
public class DBHelper {
    ...
    public boolean InsertInvoice(InvoiceClass invoice)
    {
        try
        {
            db.execSQL("insert into Invoice values('"+invoice.getInvoiceID()+"', '"+invoice.getProductID()+"', '"+invoice.getQuantity()+"', '"+invoice.getTotal()+"')");
            return true;
        }
    ...
    public Cursor SearchAllProduct()
    {
        ...
        try
        {
            cursor=db.rawQuery("Select ProductName from Product",null);
        }
